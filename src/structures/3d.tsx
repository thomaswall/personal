...
export default function THREED(props) {
  ...
  let addytoiElement = () => {
    ...
    let diff =
      (document.getElementById("ibm").clientHeight +
        document.getElementById("ytoi").clientHeight) /
  ...
  let addAstrumElement = () => {
    ...
    let object = new CSS3DObject(div);
    let diff =
      (document.getElementById("astrum").clientHeight +
        document.getElementById("ibm").clientHeight +
        document.getElementById("ytoi").clientHeight) /
      document.body.scrollHeight;
  ...
  let animate = () => {
    ...
    planemat.uniforms["time"].value = 0.00025 * (Date.now() - start);
    ...
    for (let mesh of vidGroup.children) {
      mesh.lookAt(
        new THREE.Vector3(
          (mouse.x - 0.5) * exag - 850,
          (mouse.y - 0.5) * exag + 100,
          0.5
        )
      );
      mesh.position.x =
        -200 * wratio + mesh.orig_pos.x + perc_scroll * 600 * wratio;
    }
  };
